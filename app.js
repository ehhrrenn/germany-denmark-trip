...
async function parseTextWithAI() {
    ...
    // Strict prompt to minimize conversational filler
    const prompt = `Extract itinerary details from the text below. 
    Return ONLY a raw JSON object. Do not include markdown code blocks. 
    Keys: date (YYYY-MM-DD HH:MM), location (City, Country), lat (Number), lng (Number), category (Transport, Accommodation, Food & Drink, Activity), notes (String).
...
const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
        ...
        // 1. IMPROVED CLEANUP: Use a Regular Expression to find the first '{' and last '}'
        // This ignores any conversational text or markdown the AI might have added.
        const jsonMatch = aiText.match(/\{[\s\S]*\}/);
        
        if (!jsonMatch) {
            throw new Error("AI did not return a valid data object. Try pasting more text.");
        }
